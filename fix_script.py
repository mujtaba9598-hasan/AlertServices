...
# 1. Remove the misplaced Global Trading Backend block
start_idx = content.find('      // --- Global Trading Backend ---')
if start_idx != -1:
    end_idx = content.find('      e.preventDefault();', start_idx)
    if end_idx != -1:
        content = content[:start_idx] + content[end_idx:]

# 2. Find the ACTUAL local trading logic block and replace it
# The original block starts with: const tradeForm = document.getElementById('trade-form');
# We will replace from there to the end of the event listener
...
# The old logic starts at "// --- Trading Page Logic ---" or "const tradeForm = document.getElementById('trade-form');"
# Let's find "const tradeForm = document.getElementById('trade-form');"
old_start = content.find("const tradeForm = document.getElementById('trade-form');")
if old_start != -1:
    # Find where the block ends. It's the end of the `if (tradeForm) { ... }` block
    # We can just look for the next major comment or end of file
    old_end = content.find('// Fake Admin Button Easter Egg', old_start)
    if old_end == -1:
        old_end = len(content)
        
    content = content[:old_start] + trade_logic + "\n\n" + content[old_end:]
