...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const triggerMatrixTheme = () => {
    ...
    const hasAllThemes = unlocked.includes('dice_hunter') && 
                         ...
                         unlocked.includes('system_failed');
  ...
  const achievements = [
    ...
    { id: 'beyond_alerted', title: 'Beyond Alerted', desc: 'Activated Matrix with every theme unlocked. (Unlocks White)', icon: 'fa-ghost', check: () => stats.beyondAlerted },
  ...
  document.querySelector('.footer-dice-right').addEventListener('click', function() {
    ...
    let unlockedThemesCount = 1 + unlocked.filter(id => ['dice_hunter', 'generous_spirit', 'theme_master', 'touch_lamp', 'system_failed', 'beyond_alerted'].includes(id)).length + (stats.balancedUnlocked ? 1 : 0) + 1; // +1 for white, +1 for default
